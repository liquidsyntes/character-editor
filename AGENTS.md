...
2. `docs/actual_task.md` - The current tasks and plans.
3. `docs/current-state.md` - The latest project status.

Always run `npm run lint` and `npm run build` to verify your changes.

# В конце выполнения любой задачи делай:

- Всегда! После любых измений в любом из файлов, в конце своего сообщения сгенерируй мне кратко текст для комита, на русском языке.
