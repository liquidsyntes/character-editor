...
import { getAppSetting, setAppSetting, getDefaultVoicePrompt } from '@/app/actions/settings';
import { HighlightedTextarea } from './HighlightedTextarea';

export function VoicePromptsPanel({
  isOpen,
  onClose,
}: {
  isOpen: boolean;
  ...
  useEffect(() => {
    async function load() {
      const defPrompt = await getDefaultVoicePrompt();
      setDefaultPrompt(defPrompt);
      const p = await getAppSetting('VOICE_PROMPT');
      setPrompt(p || defPrompt);
      setLoading(false);
    }
    load();
  }, []);
  ...
  const handleSave = async (value: string) => {
    setSaving(true);
    await setAppSetting('VOICE_PROMPT', value);
    setSaving(false);
  };
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-6">
          <p className="font-label-caps text-[10px] text-on-surface-variant/80 mb-6 leading-relaxed">
            Этот системный промпт определяет стиль, тон и структуру генерируемых диалогов для голоса персонажа.
          </p>
          ...
          {loading ? (
            ...
            <div className="border border-outline-variant rounded bg-surface overflow-hidden flex flex-col h-[calc(100vh-200px)]">
               <div className="w-full flex items-center p-3 bg-surface-container border-b border-outline-variant shrink-0">
                 <span className="font-label-caps text-[12px] uppercase">Промпт: Голос персонажа</span>
               </div>
